...
class LowPassFilters(torch.nn.Module):
    ...
    def __init__(self, cutoffs: list, width: int = None):
        ...
        for cutoff in cutoffs:
            sinc = torch.tensor(np.sinc(2 * cutoff * t))
...
class Remix(nn.Module):
    """Remix.
    Mixes different noises with clean speech within a given batch, clean not changed
    """

    def forward(self, sources):
        noise, clean = sources
        n_batch, *other = noise.shape
        perm = torch.argsort(torch.rand(n_batch, device=noise.device), dim=0)
...
class Shift(nn.Module):
    ...
    def forward(self, wav):
        sources, batch, length = wav.shape
        ...
        if self.shift > 0:
            if not self.training:
                ...
                offsets = torch.randint(
                    self.shift,
                    [1 if self.same else sources, batch, 1], device=wav.device)
                offsets = offsets.expand(sources, -1, -1)
