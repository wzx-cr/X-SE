...
class ERB(nn.Module):
    def __init__(self, erb_subband_1, erb_subband_2, nfft=512, high_lim=8000, fs=16000):
        super().__init__()
        high_lim = min(float(high_lim), float(fs) / 2.0)
...
class ULUNAS(nn.Module):
    """UL-UNAS waveform enhancer integrated for MOS-SE TimeSE systems."""
    ...
    def __init__(
        ...
        self.erb = ERB(erb_low, erb_high, nfft=n_fft, high_lim=high_lim, fs=sample_rate)
    ...
    def forward(self, input):
        ...
        output = torch.istft(spec_enh, **stft_kwargs, length=n_samples)
